...
class BaseDenseLayer(nn.Module):
    ...
    def initParameters(self):
        ...
        nn.init.xavier_normal(stateDict['layer.2.weight'])
        # nn.init.constant(stateDict['layer.2.bias'], 0)
        nn.init.xavier_normal(stateDict['layer.5.weight'])
...
class DenseBlock(nn.Module):
    def __init__(self, inputDim=256, outputDim=256, growthRate=32, blockDepth=8, withTrans=True):
    ...
    def initParameters(self):
        stateDict = self.state_dict()
        if self.withTrans:
            nn.init.xavier_normal(stateDict['transition.2.weight'])
